...
class StaticCacheBenchMark(BenchMark):
    ...
    def __init__(self, repo_id, prefill_num_iter=3, num_iter=100, device="cuda", attn_implementation="sdpa", all_dtype=torch.bfloat16):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(repo_id, torch_dtype=all_dtype, attn_implementation=attn_implementation, token=token).to(device, all_dtype).eval()
    ...
    def target(self, batch_size, max_cache_length, seq_length):
        ...
        input_ids = self.tokenizer([self.input_text] * batch_size, return_tensors="pt").to(self.device).input_ids[:batch_size, :seq_length]
        ...
        with torch.no_grad():
            compiled_model = torch.compile(self.model, mode="reduce-overhead", fullgraph=True)
            set_seed(0)
...
if __name__ == "__main__":
    ...
    all_max_cache_length = [2048, 1024, 512, 256]
    all_seq_length = [256, 1, 512, 1024]
